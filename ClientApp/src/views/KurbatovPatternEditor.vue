    ...
    <v-container>
         <v-row v-if="!initialDialog" class="alwaysOnTop">
            <v-col lg="2" offset-lg="1">
                <v-btn v-if="validation && patternName && smpArray.length > 0"
                       color="success"
                       block
                       @click="savePattern(smpArray)">
                    {{mode==='creating' ? 'Сохранить шаблон' : 'Обновить шаблон'}}
                </v-btn>
            </v-col>
            <v-col lg="2">
                <v-btn v-if="mode==='updating'"
                       color="pink"
                       block
                       @click="deletePattern(selectedPattern)">
                    Удалить шаблон
                </v-btn>
            </v-col>
            <v-col lg="2" offset-lg="1">
                <v-text-field v-if="mode==='creating'"
                              outlined
                              v-model="patternName"
                              :error-messages="patternName
                                        ? []
                                        : 'Введите название шаблона'" label="Название шаблона">
                </v-text-field>
                <v-select  v-else
               ...
               <v-stepper v-if="smpArray.length > 0" v-model="step" vertical>
                    <template v-for="(smp, index) in smpArray">
                        <v-stepper-step
                            :key="`${index+1}-step`"
                            :step="index + 1"
                            :color="$store.getters['smpstorage/validationIsCorrect'](smp.guid)
                                    ? 'green'
                                    : 'red'"
                            complete
                            editable>
                        ...
                        <v-stepper-content
                            ...
                            <v-card>
                                <v-card-text>
                                    <single-kp
                                        :guid="smp.guid"
                                        @chbx-dialog="showCopyDialog"
                                        @delete-smp="deleteSmp">
                                    </single-kp>
                                </v-card-text>
                            </v-card>
                        </v-stepper-content>
            ...
            <v-dialog v-model="smpCreateDialog" persistent max-width="400">
                <v-card>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn v-if="readyToCreateSMP"
                               color="indigo"
                               @click="createSmp">Добавить
                        </v-btn>
                   </v-card-actions>
                </v-card>
            </v-dialog>
                    ...
                    <v-card-text>
                        ...
                        <v-row class="mt-6" v-if="selectedDieTypeId">
                            <v-col lg="12">
                                ...
                                <v-btn v-else
                                       block
                                       @click="goToCreatingMode"
                                       color="indigo">Создать новый шаблон
                                </v-btn>
                            </v-col>
                        </v-row>
                        <v-row class="mt-6" v-if="selectedDieTypeId">
                            <v-col lg="12">
                                <v-btn v-if="mode==='updating'"
                                       block
                                       @click="changeSelectedPattern(selectedPattern)"
                                       color="indigo">Подтвердить выбор
                                </v-btn>
                                <v-btn v-else
                                       block
                                       @click="goToUpdatingMode(selectedDieTypeId)"
                                       color="indigo">Редактировать шаблон
                                </v-btn>
                            </v-col>
                        </v-row>
                    </v-card-text>
...
<script>
import singleKp from '../components/kurbatovparametersingle-view.vue';
import { createSmp as createSmpFromService, restoreFromVm as restoreFromVmFromService } from '../services/smp.service.js';
import {
  StandartMeasurementPatternFullViewModel,
  StandartPattern,
  StandartMeasurementPattern,
  KurbatovParameter,
  KurbatovParameterBorders,
} from '../models/kurbatovparameter.js';
import checkboxSelectDialog from '../components/Dialog/checkboxselect-dialog.vue';
...
export default {
  ...
  methods: {
      ...
      if (routeName === 'kurbatovparameter-updating') {
        ...
        await this.$http
          ...
          .then((response) => {
            ...
            this.selectedPattern = this.patterns.find((x) => x.id === selectedPatternId) || {};
          })
          .then(() => {
            if (!this.patterns.map((x) => x.id).includes(this.selectedPattern.id)) throw new Error();
          })
          .then(async () => this.getSelectedPattern(this.selectedPattern))
    ...
    async deletePattern(selectedPattern) {
      await this.$http({
        ...
        .then(async (pattern) => {
          _.isEmpty(pattern)
            ? await this.changeSelectedPattern(pattern)
    ...
    async fillSmpStorage() {
      await this.getProcessByDieId(this.selectedDieTypeId)
        .then(async () => this.getElementsByDieType(this.selectedDieTypeId))
        .then(async () => this.getStagesByProcessId(this.process));
  ...
  computed: {
    readyToCreateSMP() {
      return !_.isEmpty(this.selectedElementSMP)
             && !_.isEmpty(this.selectedStageSMP)
             && !_.isEmpty(this.selectedDividerSMP);
    },
  ...
  async mounted() {
    ...
    await this.initialize()
      .then(async () => this.routeHandler(this.$route.name)).then(() => this.closeLoading());
  },
