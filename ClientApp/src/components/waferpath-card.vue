<template>
    <div>
    <v-skeleton-loader v-if="_.isEmpty(dirtyCellsInfo)"
                            class="mx-auto"
                            type="card-avatar">
    </v-skeleton-loader>
    <v-card v-else color="#303030" class="mx-auto">
        <v-container>
            <v-row>
                <v-col lg="12">
                    <v-btn block color="primary" @click="$router.push({ name: 'wafermeasurement-fullselected', params: { waferId: waferId, measurementName: digit + '_' + elementName }})" outlined>{{elementName}}</v-btn> 
                </v-col>
            </v-row>
                <v-row>
                <v-col lg="6">
