...
export default {
  ...
  watch:
    {
      createDialog(value) {
        if (!value) Object.keys(this.newDevice).forEach((_) => this.newDevice[_] = '');
        this.newDevice.deviceType = this.deviceTypes[0].model;
      },
      editDialog(value) {
        if (!value) Object.keys(this.editedAddress).forEach((_) => this.editedAddress[_] = '');
      },
    },
  ...
  methods:
    {
      validate(parameter, value) {
        const validation = this.rules[parameter];
        if (!value) {
      ...
      async editAddress() {
        ...
        await this.$http({
          ...
          url: '/api/device/edit',
          data: {
            id, address, name: editedDevice.name, model: editedDevice.model,
          },
          config: {
            headers: {
