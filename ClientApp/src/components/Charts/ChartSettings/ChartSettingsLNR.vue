...
export default {
  ...
  methods: {
    ...
    validateMinMaxField(axis, value) {
      if (isNaN(axis[value]) || (axis.max < axis[value] && axis.max !== 'Авто') || (axis.min > axis[value] && axis.min !== 'Авто')) axis[value] = 'Авто';
    },

    validateTicksField(axis) {
      if (isNaN(axis.maxTicksLimit) || axis.maxTicksLimit < 4 || !Number.isInteger(+axis.maxTicksLimit)) axis.maxTicksLimit = 11;
    },
