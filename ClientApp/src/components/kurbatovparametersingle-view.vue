<template>
  <v-container>
    <v-row>
      ...
      <v-col lg="1">
        <v-btn
          v-if="validationIsCorrect"
          fab
          dark
          small
          color="indigo"
          @click="$emit('chbx-dialog', guid)"
        >
      ...
      <v-col lg="1">
        <v-btn v-if="!validationIsCorrect" fab dark small color="pink"
          ><v-icon dark color="white">error_outline</v-icon></v-btn
        >
        <v-btn v-else fab dark small color="green"
          ><v-icon dark color="white">done_outline</v-icon></v-btn
        >
      </v-col>
    </v-row>
    <v-row>
        ...
        <v-stepper :key="stepperKey" v-model="step">
            ...
            >
              <div>
                <v-row>
                  <v-col lg="3">
                    <v-select
                      :value="parameter.standartParameter"
                      :items="
                        forbiddenStandartParameters.length === 0
                          ? []
                          : standartParameters.filter(
                              x =>
                                !forbiddenStandartParameters
                                  .find(f => f.key === parameter.key)
                                  .forbiddenIds.includes(x.id)
                            )
                      "
                      no-data-text="Нет данных"
                      return-object
                  ...
                  <v-col lg="4">
                    <v-text-field
                      :value="parameter.standartParameter.russianParameterName"
                      :error-messages="
                        parameter.validationRules.parameterRq
                          ? []
                          : 'Выберите параметр'
                      "
                      readonly
                  ...
                  <v-col lg="5">
                    <v-text-field
                      :value="parameter.standartParameter.parameterNameStat"
                      :error-messages="
                        parameter.validationRules.parameterRq
                          ? []
                          : 'Выберите параметр'
                ...
                <v-row>
                  <v-col lg="2">
                    <v-text-field
                      :key="`bndl-` + parameter.key"
                      :value="parameter.bounds.lower"
                      :error-messages="
                        validationBoundsErrors(
                          parameter.validationRules,
                          `lower`
                        )
                      "
                      :readonly="!parameter.withBounds.value"
                      @change="updateBounds($event, 'lower', parameter)"
                      outlined
                  ...
                  <v-col lg="2">
                    <v-text-field
                      :key="`bndu-` + parameter.key"
                      :value="parameter.bounds.upper"
                      :error-messages="
                        validationBoundsErrors(
                          parameter.validationRules,
                          `upper`
                      ...
                      "
                      :readonly="!parameter.withBounds.value"
                      @change="updateBounds($event, 'upper', parameter)"
                      outlined
          ...
          <div class="parameter-actions">
            ...
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-btn
                  v-if="smp.kpList.length > 0"
                  fab
                  dark
                  small
                  color="indigo"
                  v-on="on"
                  @click="deleteParameter(step)"
                >
                  <v-icon dark color="primary">delete</v-icon>
                </v-btn>
              </template>
