export const exportkurb = {
  ...
  getters: {
    isReadyForExport: (state) => state.elementsReady.length > 0
                                 && state.elementsReady.every((_) => _.ready),
    isThisElementisReady: (state) => (key) => {
      const thisElement = state.elementsReady.find((_) => _.key === key);
      return thisElement && thisElement.ready;
    },
    elementsAutoIdmrStatus: (state) => state.elementsAutoIdmr,
  },
  ...
  mutations: {
    ...
    deleteFromElementsReady(state, payload) {
      state.elementsReady = state.elementsReady.filter((value) => value.key !== payload);
    },
  },
};
