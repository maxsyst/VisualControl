...
namespace VueExample.Entities.Configurations
{
    public class KurbatovParameterConfiguration : IEntityTypeConfiguration<KurbatovParameterEntity>
    {
        public void Configure(EntityTypeBuilder<KurbatovParameterEntity> builder)
        {
            ...
            builder.HasOne(k => k.StandartMeasurementPatternEntity).WithMany(k => k.KurbatovParameters).HasForeignKey(fk => fk.SmpId).OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(k => k.KurbatovParameterBordersEntity).WithMany(k => k.KurbatovParameters).HasForeignKey(fk => fk.BordersId).OnDelete(DeleteBehavior.Cascade);
        }
    }
}