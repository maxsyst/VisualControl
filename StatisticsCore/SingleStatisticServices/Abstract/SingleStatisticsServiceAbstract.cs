...
namespace VueExample.StatisticsCore.SingleStatisticServices.Abstract
{
    public abstract class SingleStatisticsServiceAbstract
    {
        ...
        {           
           var singleStatisticDataList = new ConcurrentBag<VueExample.StatisticsCore.DataModels.SingleStatisticData>();
           var singleParameterStatisticConcurrent = singleParameterStatisticList.AsParallel();
           Parallel.ForEach (statisticList, statisticsItem => 
           {
                var origin = singleParameterStatisticConcurrent.FirstOrDefault(x => x.Name == statisticsItem.StatisticsName); 
                if(origin != null)
                {
           ...
           return singleStatisticDataList.ToList();
